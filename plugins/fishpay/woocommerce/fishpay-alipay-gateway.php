...
/**
 * Fishpay Alipay Gateway.
 *
 * Provides a Cash on Delivery Payment Gateway.
 *
 * @class       Fishpay_Alipay_Gateway
 * @extends     WC_Payment_Gateway
 * @version     1.0.0
 * @package     WooCommerce\Classes\Payment
 */
class Fishpay_Alipay_Gateway extends WC_Payment_Gateway
{
    /**
     * 构造函数
     */
    ...
    {
        $this->id = 'fishpay_alipay_gateway';
        $this->icon    = apply_filters( 'woocommerce_fishpay_alipay_icon', '' );
        $this->method_title = __('鱼码-支付宝扫码', 'fishpay');
        $this->method_description = __('鱼码支付宝插件为WooCommerce.', 'fishpay');
        $this->has_fields = false;
        $this->init_form_fields();
        $this->init_settings();
        $this->enabled = $this->get_option( 'enabled' );
        $this->title = $this->get_option('title');
        $this->description   = $this->get_option( 'description' )?:__('鱼码支付宝插件为WooCommerce.', 'fishpay');
        ...
        //异步通知
        add_action('woocommerce_api_'.$this->id, array($this, 'fishpay_alipay_gateway_callback'));
        //同步
        add_action( 'woocommerce_thankyou_' . $this->id, array( $this, 'thankyou_page' ) );

    }

    function fishpay_alipay_gateway_callback()
    {
        // 处理异步通知逻辑
        $arr = $_POST;
    ...
    public function thankyou_page() {
        ...
        if ($sign == $mysign) {
            // Remove cart.
            WC()->cart->empty_cart();
            echo wp_kses_post( wpautop( wptexturize( '您的订单ID:'.$order_no.'鱼码-支付宝扫码支付成功！' ) ) );
        }
    }
    ...
    public function init_form_fields()
    {
        $this->form_fields = array(
            'enabled' => array(
                ...
                'label' => __('启用/停用鱼码-支付宝扫码支付', 'fishpay'),
                'type' => 'checkbox',
                'description' => '',
                'default' => 'no',
            ),
